...
namespace VirtualOS.OperatingSystem.Commands
{
    ...
    public class CreateDirectory : Command
    {
        ...
        public CreateDirectory(ref FileSystem fs)
        {
            _aliases = new List<string>() { "md", "mkdir" };
        ...
        {
            args.RemoveAt(0); // Removing command name
            ...
            foreach (var filename in args)
            {
                ...
                {
                    CommandLine.Error($"You can create only directories: {filename}. Skipping this path.");
                    continue;
                }
